...
// Custom Imports
import {
  getPathForFirebaseStorage,
  generateImagePublicURLFirebase,
} from '../Utility/Utils';
    ...
    if (images.length != 0) {
      ...
      images.forEach(async element => {
        ...
        try {
          ...
          const resImgPublicUrl = generateImagePublicURLFirebase(
            resImg.metadata.name,
          );
    ...
    if (videos.length != 0) {
      ...
      videos.forEach(async element => {
        ...
        try {
          ...
          setUploadedCount(ctr + 1);
          const resVidPublicUrl = generateImagePublicURLFirebase(
            resVid.metadata.name,
          );
