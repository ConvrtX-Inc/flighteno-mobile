...
export default function SupportTicket({navigation}) {
  const {loading, supportTickets, currentUser, token} = useSelector(
    ({authRed}) => authRed,
  );
          ...
          <FlatList
            ...
            renderItem={({item, index}) => (
              <View key={item._id}>
                <TouchableOpacity
                  onPress={() =>
                    navigation.navigate('SupportReply', {ticket: item})
                  }>
                  <View
                    style={{
                      borderRadius: 20,
                    ...
                    <View style={Styles.suportFlattxtView}>
                            ...
                            Styles.suportno,
                            {
                              color:
                                item.status == 'pending'
                                  ...
                                  : '#10CF73',
                              marginLeft: 'auto',
                              marginRight: (windowWidth * 5.3) / 100,
                      ...
                      <TextMedium numberOfLines={2} style={Styles.suportTxt}>
                        {item.message}
                      </TextMedium>
                      <TextMedium style={Styles.suportTxtDate}>
                        {moment(
                          item.created_date.$date.$numberLong,
                          'x',
                        ).format('MM/DD/YYYY')}
                      </TextMedium>
                    </View>
