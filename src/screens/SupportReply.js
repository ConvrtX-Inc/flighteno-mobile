...
export default function SupportReply({navigation, route}) {
  ...
  const {currentUser, loading, supportTickets} = useSelector(
    ({authRed}) => authRed,
  );
    ...
    RNFetchBlob.config({
      ...
      .then(res => {
        ...
        android.actionViewIntent(
          `${dirs.DownloadDir}/${result}.${extension}`,
          newExt,
        );
      });
  };
          ...
          <View style={Styles.SuportReplyCSS}>
            <View style={Styles.replyUperMainBox}>
              ...
              <View style={Styles.replyHTxtBox}>
                <TextMedium style={Styles.replyHTxt}>{ticket._id}</TextMedium>
                <TextBold style={[Styles.replyHTxt, {marginVertical: 2}]}>
                  {currentUser.full_name}
                </TextBold>
                <TextMedium style={[Styles.replyHTxt, {color: '#8E8E8E'}]}>
                  {moment(ticket.created_date.$date.$numberLong, 'x').format(
                    'MM/DD/YYYY | hh:mmA',
                  )}
                </TextMedium>
              </View>
              <View style={Styles.replyStatusBox}>
                <TextMedium
                  style={[
                    ...
                    {
                      marginTop: 0,
                      color: ticket.status == 'pending' ? '#FFA800' : '#10CF73',
                      marginLeft: 'auto',
            ...
            <TextMedium
              style={[
                Styles.suportTxt,
                {
                  ...
                  lineHeight: (windowWidth * 5.5) / 100,
                  marginTop: (windowWidth * 5.8) / 100,
                },
              ]}>
              {ticket.message}
            </TextMedium>
            <FlatList
              ...
              renderItem={({item, index}) => (
                <TouchableHighlight
                  ...
                  onPress={() => toggleSlider(index, ticket.image)}>
              ...
              <View style={Styles.SuportReplyCSS}>
                <View style={Styles.replyUperMainBox}>
                  ...
                  <View style={Styles.replyHTxtBox}>
                    ...
                    <TextMedium style={[Styles.replyHTxt, {color: '#8E8E8E'}]}>
                      {moment(
                        element.created_date.$date.$numberLong,
                        'x',
                ...
                {element.image ? (
                  <TouchableHighlight
                    style={{marginTop: 10}}
                    underlayColor="transparent"
                    onPress={() => toggleSlider(0, [element.image])}>
                    <Image
                ...
                {element.file ? (
                  <TouchableOpacity
                    activeOpacity={0.5}
                    onPress={() => downloadFile(element.file)}
                    style={{
                      height: 40,
                      width: 150,
                      flexDirection: 'row',
